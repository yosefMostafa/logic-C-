...
void Addswitch::Execute(string s)
{
	...
	if (pGInfo->PointsList[0].y > (pUI->getToolBarHeight()) + 15 &&
		...
		Switch* pA = new Switch(pGInfo, AND2_FANOUT, true, s);
...
void Addswitch::Undo(GraphicsInfo* rGInfo)
{
...
void Addswitch::Redo(GraphicsInfo* pGInfo)
{
	UI* pUI = pManager->GetUI();
	if (pGInfo->PointsList[0].y > (pUI->getToolBarHeight()) + 15 &&
		pGInfo->PointsList[1].y < (pUI->getheight()) - (pUI->getStatusBarHeight()) &&
